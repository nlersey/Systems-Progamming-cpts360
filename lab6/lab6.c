  ...
  dir[0] = "/bin";
   ...
   while(1){
    ...
    if(strcmp(token[0],"cd")==0)
    {
      ...
      {
        chdir(dhome);//not sure if will work?
      }
      continue;
    }
    if(strcmp(token[0],"exit")==0)
    {
     ...
     pid = fork();
      ...
      {      // child sh
       ...
       //function for pipe checking
       flag=pipecheck(n);
       //return some int result, 1 if pipe, 0 if no pipe
        
        if(flag==1)
        {
            //dopipe function
            //if()
            break;//need to make sure this works
        }
        else if(flag==0)
        {
            for(int j=0;j<n;j++)
          {
            //printf("\n inside i/o \n");
            if(strcmp(token[j],">")==0)//send to outfile
            {
              printf("inside >\n");
              close(1);
              int fd = open(token[j+1], O_WRONLY|O_CREAT, 0644);
              token[j] = NULL;      
              break;
           ...
           if(strcmp(token[j],">>")==0)//append to out
            {
              printf("inside >>\n");
             close(1);
              int fd = open(token[j+1], O_WRONLY|O_CREAT|O_APPEND, 0644);
              token[j] = NULL;
              //might need call to du() here

            }
           if(strcmp(token[j],"<")==0)//take from infile
            {
              printf("inside <\n");
             close(0);
              int fd = open(token[j+1], O_RDONLY);
              token[j]=NULL;
          ...
          for(int x=0;x<ndir;x++)
          {
            ...
            strcat(cmd, token[0]);
            printf("cmd=%s\n", cmd);
            int r = execve(cmd, token, env);//replaces current child process with new process creating by running program
...
int pipeexecution(int ndir, char* dir, char *cmd,char *env, int *pd)
{
if(pd)
{
